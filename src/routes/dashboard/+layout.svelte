...
<div
  id="figtree"
  class="relative min-h-screen flex flex-col {prefersDarkMode
    ? 'bg-gray-900 text-gray-100'
    : 'bg-gray-50 text-gray-800'}"
>
  <!-- Top Navbar -->
  <header
    class="z-30 flex items-center {prefersDarkMode
      ? 'bg-gray-800 shadow-gray-900/50'
      : 'bg-white shadow-gray-100/80'} 
    backdrop-blur-md shadow-sm px-4 py-3 sticky top-0 h-16 transition-colors duration-300"
    in:fade={{ duration: 300 }}
  >
    <!-- Mobile Menu Button -->
    <button
      class="mr-4 {prefersDarkMode
    ...
    <div class="flex items-center">
      <div
        class="hidden lg:flex w-9 h-9 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl items-center justify-center mr-3 text-white font-bold text-xl shadow-lg shadow-blue-500/20"
      >
        PB
      </div>
      <h1
        class="text-lg font-semibold tracking-tight {prefersDarkMode
          ? 'text-white'
          : 'text-blue-800'}"
      >
    ...
    <div class="hidden md:flex mx-auto max-w-md w-full px-4 relative">
      <div
        class="relative w-full rounded-full overflow-hidden {searchFocused
          ? 'ring-2'
          : ''} 
        {prefersDarkMode
          ? 'ring-blue-500/50 bg-gray-700/50'
          : 'ring-blue-300 bg-gray-50'} 
        transition-all duration-200"
      >
        <input
          type="text"
          placeholder="Zoeken naar boekingen, klanten..."
          class="w-full px-4 py-2 pr-10 border-none outline-none {prefersDarkMode
            ? 'bg-gray-700/50 text-white placeholder-gray-400'
            : 'bg-gray-50 text-gray-800 placeholder-gray-500'} text-sm"
          on:focus={() => (searchFocused = true)}
          on:blur={() => (searchFocused = false)}
        />
        <div
          class="absolute right-3 top-2.5 {prefersDarkMode
            ? 'text-gray-400'
            : 'text-gray-500'}"
        >
          <svg
    ...
    <div class="ml-auto flex items-center space-x-2 sm:space-x-3">
      ...
      <button
        aria-label="search"
        class="relative p-2 {prefersDarkMode
          ? 'text-gray-300 hover:bg-gray-700'
          : 'text-gray-600 hover:bg-gray-100'} rounded-full transition-colors md:hidden"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="20"
      ...
      <div class="relative">
        <button
          ...
          class="relative p-2 {prefersDarkMode
            ? 'text-gray-300 hover:bg-gray-700'
            : 'text-gray-600 hover:bg-gray-100'} rounded-full transition-colors"
        ...
        {#if notificationDropdownOpen}
          <div
            class="absolute z-20 border overflow-hidden {prefersDarkMode
              ? 'bg-gray-800 border-gray-700'
              : 'bg-white border-gray-200'} rounded-xl shadow-xl py-1.5"
          ...
          >
            <div
              class="flex justify-between items-center px-4 py-3 border-b {prefersDarkMode
                ? 'border-gray-700'
                : 'border-gray-100'}"
            >
              <h3
                class="text-sm font-medium {prefersDarkMode
                  ? 'text-white'
                  : 'text-gray-900'}"
              >
                Notificaties
              ...
              <span
                class="px-2 py-0.5 rounded-full text-xs font-medium {prefersDarkMode
            ...
            <div
              class="divide-y divide-gray-100 dark:divide-gray-700 max-h-[60vh] overflow-y-auto"
            >
              {#if unreadNotifications.length === 0}
                <div class="p-4 text-center">
                  <p
                    class="text-sm {prefersDarkMode
                      ? 'text-gray-400'
                      : 'text-gray-500'}"
                  >
              ...
              {:else}
                {#each unreadNotifications.slice(0, 3) as notification (notification.id)}
                  ...
                  >
                    <div class="flex">
                      <div
                        class="{getNotificationTypeClass(
                          notification.type,
                      ...
                      <div class="ml-3 flex-1 min-w-0">
                        <div class="flex justify-between">
                          ...
                          <p
                            class="text-xs {prefersDarkMode
                              ? 'text-gray-400'
                              : 'text-gray-500'} ml-2 flex-shrink-0"
                          >
                            {notification.time}
                          </p>
                        </div>
                      ...
                      <div class="flex-shrink-0 ml-2">
                        <div
                          class="w-2 h-2 rounded-full {prefersDarkMode
                            ? 'bg-blue-400'
                            : 'bg-blue-600'}"
                        ></div>
                      </div>
            ...
            <div
              class="p-3 {prefersDarkMode ? 'bg-gray-900/20' : 'bg-gray-50'}"
            >
              <button
                class="w-full text-sm font-medium {prefersDarkMode
      ...
      <div class="relative">
        <button
          ...
          class="flex items-center space-x-2 p-1.5 {prefersDarkMode
            ? 'hover:bg-gray-700'
            : 'hover:bg-gray-100'} 
        ...
        {#if profileDropdownOpen}
          <div
            class="absolute right-0 mt-2 w-48 {prefersDarkMode
              ? 'bg-gray-800 border-gray-700'
          ...
          >
            <div
              class="border-b {prefersDarkMode
                ? 'border-gray-700'
                : 'border-gray-100'} pb-2 pt-1 px-4"
            >
              <p
                class="text-xs {prefersDarkMode
                  ? 'text-gray-400'
              ...
              <p
                class="font-medium {prefersDarkMode
                  ? 'text-white'
                  : 'text-gray-800'}"
              >
                Beheerder
            ...
            <a
              ...
              class="flex items-center px-4 py-2 text-sm {prefersDarkMode
                ? 'text-gray-300 hover:bg-gray-700'
                : 'text-gray-700 hover:bg-gray-100'}"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="w-4 h-4 mr-3 {prefersDarkMode
                  ? 'text-gray-400'
                  : 'text-gray-500'}"
                viewBox="0 0 24 24"
              >
            ...
            >
              ...
              >
                <path
                  fill="currentColor"
                  d="M12 4a4 4 0 1 0 0 8a4 4 0 0 0 0-8M6 12a6 6 0 0 0-6 6c0 1.1.9 2 2 2h20a2 2 0 0 0 2-2a6 6 0 0 0-6-6z"
                />
              </svg>
              Profiel
            </a>
            <hr
              class="my-1 {prefersDarkMode
                ? 'border-gray-700'
                : 'border-gray-100'}"
            />
            <button
              on:click|stopPropagation={logout}
              class="flex w-full items-center px-4 py-2 text-sm {prefersDarkMode
  ...
  <div class="flex flex-1 overflow-hidden relative">
    ...
    <aside
      class="fixed top-16 bottom-0 left-0 w-[280px] {prefersDarkMode
        ? 'bg-gray-800'
        : 'bg-white'} 
        shadow-lg z-20 transform transition-transform duration-300 ease-in-out
        {sidebarOpen
        ? 'translate-x-0'
        : '-translate-x-full'} lg:translate-x-0 flex flex-col overflow-hidden"
    >
      ...
      <div class="flex-1 overflow-y-auto p-4">
        ...
        <!-- Divider -->
        <div
          class="h-px {prefersDarkMode ? 'bg-gray-700' : 'bg-gray-200'} my-4"
        ></div>

        <!-- Quick Actions Section -->
        <div class="px-3 py-2">
          <h3
            class="text-xs font-semibold {prefersDarkMode
              ? 'text-gray-400'
          ...
          <div class="mt-3 grid grid-cols-2 gap-2">
            ...
            >
              ...
              >
                <path
                  fill="currentColor"
                  d="M12 22c-5.52 0-10-4.48-10-10S6.48 2 12 2s10 4.48 10 10-4.48 10-10 10m0-18c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8-3.58-8-8-8m1 11h-2v-6h2v6m0-8h-2V5h2v2"
                />
              </svg>
            ...
            >
              ...
              >
                <path
                  fill="currentColor"
                  d="M13 11h5v2h-5v-2m-1 2v-2H7v2h5m0-6v2H7V7h5m6 0h-5v2h5V7M5 5v14h14V5H5Z"
                />
              </svg>
        ...
        <div class="px-3 py-2 mt-2">
          <h3
            class="text-xs font-semibold {prefersDarkMode
              ? 'text-gray-400'
              : 'text-gray-500'} uppercase tracking-wider flex items-center justify-between"
          >
          ...
          <div class="mt-2 space-y-2">
            {#if recentActivities.length === 0}
              <div
                class="{prefersDarkMode
                  ? 'bg-gray-700/30'
                  : 'bg-gray-50'} p-2 rounded-lg text-xs"
              >
                <p
                  class="{prefersDarkMode
                    ? 'text-gray-300'
        ...
        >
          <div class="flex items-start">
            ...
            >
              <path
                fill="currentColor"
                d="M11.95 17.95q.65 0 1.075-.425t.425-1.075q0-.65-.425-1.075t-1.075-.425q-.65 0-1.075.425t-.425 1.075q0 .65.425 1.075t1.075.425m0-5.5q.65 0 1.075-.425t.425-1.075q0-.65-.425-1.075t-1.075-.425q-.65 0-1.075.425t-.425 1.075q0 .65.425 1.075t1.075.425M4.5 22q-.625 0-1.062-.438T3 20.5v-17q0-.625.438-1.062T4.5 2h15q.625 0 1.062.438T21 3.5v17q0 .625-.438 1.062T19.5 22z"
              />
            </svg>
            <div>
              <h3 class="text-sm font-medium">Hulp nodig?</h3>
              <p
                class="text-xs mt-1 {prefersDarkMode
                  ? 'text-blue-200/70'
                  : 'text-blue-700/80'}"
              >
              ...
              <a
                ...
                class="inline-flex items-center mt-2 text-xs font-medium hover:underline"
              >
                Contact support
      ...
      <div
        class="p-4 {prefersDarkMode
          ? 'bg-gray-800'
          : 'bg-white'} border-t {prefersDarkMode
          ? 'border-gray-700'
          : 'border-gray-200'} mt-auto safe-bottom"
      >
        <button
          on:click={logout}
    ...
    >
      {#if loading}
        ...
        >
          <div class="relative">
            <div
              class="w-12 h-12 rounded-full border-2 border-t-transparent border-l-transparent
              {prefersDarkMode
                ? 'border-blue-500'
                : 'border-blue-600'} animate-spin"
            ></div>
            <div
              class="w-12 h-12 rounded-full border-2 border-t-transparent border-r-transparent
              {prefersDarkMode
                ? 'border-blue-300'
                : 'border-blue-400'} animate-pulse absolute top-0 left-0"
              style="animation-duration: 1.5s;"
            ></div>
          </div>
          <p
            class="ml-4 font-medium {prefersDarkMode
              ? 'text-gray-400'
              : 'text-gray-500'}"
          >
            Laden...
      ...
      <div
        class="fixed bottom-0 left-0 right-0 h-16 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700
          flex items-center justify-around z-20 lg:hidden {prefersDarkMode
          ? 'bg-gray-800 border-gray-700'
          : 'bg-white border-gray-200'} safe-bottom"
      >
        <a
          href="/dashboard"
        ...
        >
          ...
          <span class="absolute top-1 right-5 w-2 h-2 rounded-full bg-blue-500"
          ></span>
        </a>
...
{#if showActivityModal}
  ...
  >
    ...
    >
      <!-- Modal header -->
      <div
        class="flex justify-between items-center p-5 border-b border-gray-100 dark:border-gray-700"
      >
      ...
      <!-- Modal body -->
      <div
        class="overflow-y-auto flex-1 divide-y divide-gray-100 dark:divide-gray-700"
      >
        ...
        {:else}
          {#each activities as activity (activity.id)}
            <div
              class="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors"
            >
              <div class="flex">
                <div
                  class="p-2 rounded-lg flex-shrink-0 {getModalActivityTypeClass(
                    activity.type,
                  )}"
                >
