<script>
    ...
    function endDrag() {
      ...
      const visibleVideo = document.elementFromPoint(
        window.innerWidth/2, 
        window.innerHeight/2
      )?.closest('.card')?.querySelector('video');
  ...
  <!-- Video Carousel -->
  <div class="mb-16">
    <h2 class="crazyfont text-center text-3xl md:text-4xl text-white mb-8">
  ...
  <style>
    ...
    .carousel {
      ...
      scrollbar-color: rgba(110, 90, 255, 0.5) transparent;
      cursor: grab;
    ...
    .carousel::-webkit-scrollbar-thumb {
      background: rgba(110, 90, 255, 0.5);
      border-radius: 4px;
    }
    ...
    .card {
      ...
      position: relative;
      overflow: hidden;
      background: linear-gradient(145deg, rgba(42, 20, 70, 0.95) 0%, rgba(10, 26, 47, 0.95) 100%);
