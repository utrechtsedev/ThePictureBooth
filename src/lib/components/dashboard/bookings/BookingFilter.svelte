...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-3 sm:p-4 border border-gray-100 dark:border-gray-700">
  <div class="flex flex-col md:flex-row gap-3 md:gap-4">
    <!-- View Toggle -->
    <div class="flex rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden w-full md:w-auto">
      <button 
        class="px-3 md:px-4 py-2 text-sm font-medium flex items-center justify-center flex-1 md:flex-initial {selectedView === 'list' 
          ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400' 
          : 'bg-white text-gray-700 hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700'}"
        on:click={() => handleViewChange('list')}
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1 md:mr-1.5" viewBox="0 0 24 24">
          <path fill="currentColor" d="M3 4h18v2H3V4zm0 7h18v2H3v-2zm0 7h18v2H3v-2z"/>
        </svg>
        Lijst
      </button>
      <button 
        class="px-3 md:px-4 py-2 text-sm font-medium flex items-center justify-center flex-1 md:flex-initial {selectedView === 'calendar' 
          ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400' 
          : 'bg-white text-gray-700 hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700'}"
    ...
    <!-- Status Filter -->
    <div class="grid grid-cols-2 md:flex md:space-x-2 md:overflow-x-auto md:pb-1 md:flex-wrap md:flex-nowrap gap-2 md:gap-0">
      <button 
