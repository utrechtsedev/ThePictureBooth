<script>
  ...
  async function toggleTaskComplete(id) {
    ...
    try {
      // IMPORTANT FIX: Use public_id instead of id for API requests
      // This ensures we're using the correct ID that the server recognizes
      const taskId = task.public_id || task.id;
    ...
    {:else}
      ...
      {#if hasCompletedTasks}
        <div class="p-4 bg-gray-50 dark:bg-gray-900/20">
          <button 
            class="w-full text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 flex items-center justify-center"
            on:click={() => showCompleted = !showCompleted}
          >
            <span>{showCompleted ? 'Verberg' : 'Toon'} voltooide taken ({completedTasks.length})</span>
            <svg 
              class="w-4 h-4 ml-1 transition-transform duration-200 ease-in-out" 
              style={showCompleted ? "transform: rotate(180deg);" : ""}
              fill="none" 
              stroke="currentColor" 
              viewBox="0 0 24 24"
            >
